					...
					<v-row class="ma-0 admin_table_area">
						...
						>
							<AdminItemsTableItem
								...
								title="*구분"
								:select="admin_notice_divisionItems"
							/>
						</v-col>
						<v-col
							cols="12"
...
export default {
	...
	computed: {
		...
		allowValue() {
			const checkValue = ['division', 'important']
	...
	methods: {
		...
		startDataMapping() {
			this.models = {
				division: this.admin_notice_divisionItems.find(item => item.value === this.editItem.notice_kind),
		...
		async onApiCallItem() {
			...
			const params = {
				notice_kind: this.models.division?.value || '', // 1:공지사항(일반사용자)  2:동화뉴스,  3: 공지사항(인테리어점)
