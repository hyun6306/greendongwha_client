...
export default {
	...
	data: () => ({
		...
		headers: [
			...
			{ text: '방 면적(㎡)', value: 'area', width: '80px' },
			{ text: '방 개수(개)', value: 'roomcount', width: '80px' },
	...
	methods: {
		...
		async getList(params = {}) {
			...
			await this.ADMIN_ACT_ESTIMATE_ROOM_AREA(items).then(res => {
				this.desserts = res
				this.loading = false
			})
		},
		searchItems() {
			const params = {
				...
				modify_date: this.models.registDate ? this.common_dateFilter(this.models.registDate) : '',
			}
		...
		async removeData() {
			...
			const params = this.model.map(item => {
				return { exclusive_area: item.exclusive_area, roomcount: item.roomcount }
			})
