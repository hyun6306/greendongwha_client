		...
		>
			...
			>
				<v-col
					...
					class="pa-0"
				>
					<h2 class="content_title">일반 회원가입</h2>
				</v-col>
				...
				>
					...
					<v-col class-name="pa-0">
						<v-tabs-items v-model="tab">
							...
							<!-- step03 (정보입력) -->
							<v-tab-item>
								<LoginSignupUser03UserInfo v-model="allowTab" />
							</v-tab-item>

							<!-- step04 (가입완료) -->
							<v-tab-item>
								<LoginSignup04Success v-model="allowTab" />
							</v-tab-item>
						</v-tabs-items>
					</v-col>
				</v-col>
...
export default {
	...
	mounted() {
		if (this.naver_code?.access_token) {
			this.naverGetToken = this.naver_code.access_token
			this.allowTab = 1
			this.tab = 1
		}
	},
